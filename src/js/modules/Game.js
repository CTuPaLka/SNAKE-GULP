// import * as MainFnc from "./Main.js";
...
export class Game {
	...
	start() {
		if (this.status.isPaused()) {
			...
			// setInterval(this.counter.showScore(), 1000);
		}
		if (!this.messageEl.innerText == "") {
	...
	pause() {
		if (this.status.isPlaying()) {
			...
			// clearInterval(this.counter.showScore());
		}
	};
	...
	doTick() {
		...
		if (this.isGameLost()) {
			...
			this.status.setPaused();
			return
		} else if (this.board.isHeadOnFood()) {
		...
		this.food.setFood();
		this.board.renderSnake();
		this.counter.showScore(this.snake.body.length);
	}
